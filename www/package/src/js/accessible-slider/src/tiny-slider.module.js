...
export var tns = function(options) {
  ...
  function initStructure () {
    ...
    // False-positive Chrome accessibility error for this aria-roledescription, see https://github.com/GoogleChrome/lighthouse/issues/9113 
    ...
    forEach(slideItems, function(item, i) {
      ...
      setAttrs(item, {
        'role': 'group',
        'aria-hidden': 'true',
        // Chrome 81 Lighthouse 5.7.0 false-positive fail, see https://github.com/GoogleChrome/lighthouse/issues/9113
  ...
  function initTools () {
    ...
    if (hasNav) {
      ...
      // generated nav
        ...
        for (var i = 0; i < slideCount; i++) {
          ...
          navHtml += '<button data-nav="' + i +'" tabindex="-1" aria-pressed="false" aria-disabled="false" aria-controls="' + slideId + '" ' + hiddenStr + ' aria-label="' + navStr + (i + 1) +'"></button>';
        }
        navHtml = '<div class="tns-nav" role="group" aria-label="' + getString('PAGINATION') + '">' + navHtml + '</div>';
      ...
      navItems[navCurrentIndex].setAttribute('aria-pressed', 'true');
    ...
    if (hasAutoplay) {
      ...
      var isPressedForPause = autoplay ? 'false' : 'true';
      
      if (autoplayButton) {
        setAttrs(autoplayButton, {
          'data-action': txt,
          'aria-label': getString('STOP_AUTOPLAY')
        });
      }
      else if (options.autoplayButtonOutput) {
        var currentState =  autoplay ? 'playing' : 'stopped';
        controllerWrapper.insertAdjacentHTML(getInsertPosition(options.autoplayPosition), '<button class="tns-autoplay" data-action="' + txt + '" aria-label="' + getString('STOP_AUTOPLAY') + '" aria-pressed="' + isPressedForPause + '" >' + txt + '</button>');
  ...
  function updateNavStatus () {
    ...
    if (nav) {
      ...
      if (navCurrentIndex !== navCurrentIndexCached) {
        ...
        navPrev.setAttribute('aria-pressed', 'false');
        navPrev.setAttribute('aria-disabled', 'false');

        setAttrs(navCurrent, {'aria-label': navStr + (navCurrentIndex + 1) + navStrCurrent});
        removeAttrs(navCurrent, 'tabindex');
        addClass(navCurrent, navActiveClass);
        navCurrent.setAttribute('aria-pressed', 'true');
